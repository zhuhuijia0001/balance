...
#define HAL_UART_Px_SEL_M          0x60         // Peripheral I/O Select for Master: /MO/CLK.
...
#if SPI_MODE == USE_HARD_SPI
static void LCD_WrByte(unsigned char c)
{
  	UxDBUF = c;
	while (UxCSR & (1 << 0));
}
...
{  
#if SPI_MODE == USE_HARD_SPI
  	P1SEL &= 0x7b; //让 P1.2  P1.7为普通IO口           01111011
    P1DIR |= 0x84; //把 P1.2 1.7设置为输出                 10000100
