...
static CONST uint8 simpleProfilechar3UUID[ATT_BT_UUID_SIZE] =
{ 
  LO_UINT16(SIMPLEPROFILE_CHAR3_UUID), HI_UINT16(SIMPLEPROFILE_CHAR3_UUID)
};

// Characteristic 4 UUID: 0xFFF4
static CONST uint8 simpleProfilechar4UUID[ATT_BT_UUID_SIZE] =
{ 
  LO_UINT16(SIMPLEPROFILE_CHAR4_UUID), HI_UINT16(SIMPLEPROFILE_CHAR4_UUID)
};

// Characteristic 5 UUID: 0xFFF5
static CONST uint8 simpleProfilechar5UUID[ATT_BT_UUID_SIZE] =
{ 
  LO_UINT16(SIMPLEPROFILE_CHAR5_UUID), HI_UINT16(SIMPLEPROFILE_CHAR5_UUID)
};

// Characteristic 6 UUID: 0xFFF6
static CONST uint8 simpleProfilechar6UUID[ATT_BT_UUID_SIZE] =
{ 
...
static uint8 simpleProfileChar6UserDesp[] = "Characteristic 6";
 ...
 * @return  bStatus_t
 */
bStatus_t SimpleProfile_GetParameter( uint8 param, void *value, uint8 *pRetLen)
{
  bStatus_t ret = SUCCESS;
  switch ( param )
  {
                            ...
                            uint8 *pValue, uint8 *pLen, uint16 offset, uint8 maxLen )
{
  ...
  if ( pAttr->type.len == ATT_BT_UUID_SIZE )
  {
    ...
    TRACE("Read attr, uuid:0x%04X, data:0x%02X\r\n", uuid, *pAttr->pValue);
                                 ...
                                 uint8 *pValue, uint8 len, uint16 offset )
{
  ...
  if ( pAttr->type.len == ATT_BT_UUID_SIZE )
  {
    ...
    TRACE("Write attr, uuid:0x%04X, offset:%d, len:%d\r\n", uuid, offset, len);	
